...
def count_string_length(non_sb_data):
    ...
    assigning a value of 6 for each 'N' instead of 1."""
    
    # Extract the last column (assuming it's a string column)
    last_column = non_sb_data.iloc[:, -1]
    
    # Calculate the adjusted length for each row
    ...
    string_lengths = last_column.apply(adjusted_length)
...
non_sb_data = load_non_sb_file(sys.argv[1])

sb_data = load_sb_file(sys.argv[2])
...
for i_event in range(len(non_sb_data)):
    for i_feature in range(0,14):
        probabilities_feature = sb_data[i_event][0][..., i_feature][0]
...
'''
print(new_df.to_csv(sep='\t', index=False, header=True), end='')
